<?php
    ...
    $adminRow = $myAdmin->getRow("*", "no = ". $_SESSION['admin_no']);

    $adminRow['permission'] = $myRole->getValue("permission", "no = ". $adminRow['role_no']);
    $adminRow['permissionList'] = explode(",", $adminRow['permission']);

    $basename = basename($_SERVER['SCRIPT_NAME']);

    $script = str_replace(array('_make'), "", $basename);

    $adminRow['permissionList'][] = 'index.php';

    if( !in_array($script, $adminRow['permissionList']) &&  $_SESSION['admin_no'] != 1)
    {
        //JavaScript::alert("没有权限");
