<?php
    ...
    $is_show = !empty($_REQUEST["is_show"]) ? $_REQUEST["is_show"] : "";
    ...
    $myTemplate = new Template(TEMPLATE_DIR ."/brand.html");
    ...
    $total_count = $myBrand->getCount($condition);
    $total_page = $myBrand->getPageCount($page_size, $condition);

    $total_page = ($total_page == 0) ? 1 : $total_page;
    $page = ($total_page < $page) ? $total_page : $page;

    // list
    $rows = $myBrand->getPage("*", $page, $page_size, $condition ." ORDER BY ". str_replace("-", " ", $order));
