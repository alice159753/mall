<?php
    ...
    $myTemplate = new Template(TEMPLATE_DIR ."/user_label.html");
    ...
    $total_count = $myUserLabel->getCount($condition);
    $total_page = $myUserLabel->getPageCount($page_size, $condition);

    $total_page = ($total_page == 0) ? 1 : $total_page;
    $page = ($total_page < $page) ? $total_page : $page;

    // list
    $rows = $myUserLabel->getPage("*", $page, $page_size, $condition ." ORDER BY ". str_replace("-", " ", $order));
    ...
    {   
        ...
        $user_num = $myUser->getCount("user_label_no = ".$rows[$i]['no']);

        $dataArray['{user_num}'] = $user_num;
