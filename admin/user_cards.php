<?php
    ...
    $myTemplate = new Template(TEMPLATE_DIR ."/user_cards.html");
    ...
    $total_count = $myUserCards->getCount($condition);
    $total_page = $myUserCards->getPageCount($page_size, $condition);

    $total_page = ($total_page == 0) ? 1 : $total_page;
    $page = ($total_page < $page) ? $total_page : $page;

    // list
    $rows = $myUserCards->getPage("*", $page, $page_size, $condition ." ORDER BY ". str_replace("-", " ", $order));
    ...
    {   
        ...
        $dataArray['{user_num}'] = $myUserCardsRecord->getCount("user_cards_no = ".$rows[$i]['no']);
