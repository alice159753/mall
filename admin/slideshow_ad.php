<?php
    ...
    $myTemplate = new Template(TEMPLATE_DIR ."/slideshow_ad.html");
    ...
    $total_count = $mySlideshowad->getCount($condition);
    $total_page = $mySlideshowad->getPageCount($page_size, $condition);

    $total_page = ($total_page == 0) ? 1 : $total_page;
    $page = ($total_page < $page) ? $total_page : $page;

    // list
    $rows = $mySlideshowad->getPage("*", $page, $page_size, $condition ." ORDER BY ". str_replace("-", " ", $order));

    $random = time();

    for($i = 0; isset($rows[$i]); $i++)
    {   
        $dataArray = $mySlideshowad->getData($rows[$i]);
