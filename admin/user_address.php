<?php
    ...
    $order = !empty($_REQUEST["order"]) ? $_REQUEST["order"] : "is_default desc";
    ...
    $myTemplate = new Template(TEMPLATE_DIR ."/user_address.html");
    ...
    $total_count = $myUserAddress->getCount($condition);
    $total_page = $myUserAddress->getPageCount($page_size, $condition);

    $total_page = ($total_page == 0) ? 1 : $total_page;
    $page = ($total_page < $page) ? $total_page : $page;

    // list
    $rows = $myUserAddress->getPage("*", $page, $page_size, $condition ." ORDER BY ". str_replace("-", " ", $order));

    $random = time();

    for($i = 0; isset($rows[$i]); $i++)
    {   
        $dataArray = $myUserAddress->getData($rows[$i]);
